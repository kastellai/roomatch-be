...
exports.getUserData = async updates => {
  return {
    ...
    wholikesme: updates.roomId.roomId
      ? await getUserInfo(updates.wholikesme)
      : await getRoomInfo(updates.wholikesme),
    ilike: updates.roomId.roomId
      ? await getUserInfo(updates.ilike)
      : await getRoomInfo(updates.ilike),
    matches: updates.roomId.roomId
      ? await getUserInfo(updates.matches)
      : await getRoomInfo(updates.matches),
...
const updatedUserMatches = async function (userIlike, roomIlike) {
  ...
  const updatedUserNewMatches = [
    ...userIlike.newMatch,
    roomIlike._id.toString(),
  ];
  ...
  await User.findById({ _id: roomIlike.roomOwner }).then(async result => {
    let newNewMatch = [...result.newMatch, userIlike._id.toString()];
    let newNewLike = result.newLike.filter(
      id => id !== userIlike._id.toString()
    );
    await User.updateOne(
